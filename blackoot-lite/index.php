	...
	edit_post_link(__('Edit', 'blackoot-lite'), '<span class="editlink"><i class="fa fa-pencil"></i>', '</span>');
	 ...
	 ?></div><?php // End metadata
		...
		if ( null != get_next_posts_link() ):
			?><div class="previous navbutton"><?php next_posts_link( '<i class="fa fa-angle-double-left"></i>' . __('попередні роботи', 'blackoot-lite') ); ?></div><?php
			endif;
			if ( null != get_previous_posts_link() ):
			?><div class="next navbutton"><?php previous_posts_link( __('наступні роботи', 'blackoot-lite') . '<i class="fa fa-angle-double-right"></i>' ); ?></div><?php
			endif;
		?></div><?php
