...
function blackoot_excerpt_more( $more ) {
	global $post;
	return '... <div class="read-more navbutton"><a href="'. get_permalink( get_the_ID() ) . '">'. __("Дивитись далі", 'blackoot-lite') .'<i class="fa fa-angle-double-right"></i></a></div>';
}
	...
	if ("" != get_adjacent_post( false, "", false ) || "" != get_adjacent_post( false, "", true ) ):
		...
		if ("" != get_adjacent_post( false, "", false ) ): // Is there a previous post?
			echo '<div class="next navbutton">',
				next_post_link('%link', __('Наступна робота', 'blackoot-lite') . '<i class="fa fa-angle-double-right"></i>' ),
				'</div>';
		endif;

		if ("" != get_adjacent_post( false, "", true ) ): // Is there a next post?
			echo '<div class="previous navbutton">',
				previous_post_link('%link', '<i class="fa fa-angle-double-left"></i>' . __('Попередня робота', 'blackoot-lite') ),
				'</div>';
		endif;
